...
node_tree = bpy.data.node_groups.new(name="Debug_Tree_12", type='GeometryNodeTree')
...
print("ObjectInfo input default value test:")
try:
    node_info.inputs[0].default_value = bpy.context.active_object
    print("Direct object assignment succeeded!")
except Exception as e:
    print("Direct object assignment failed:", e)

# What are NodeGroupOutput inputs?
for idx, inp in enumerate(node_out.inputs):
    print(f"NodeGroupOutput Input {idx}: name='{inp.name}', identifier='{inp.identifier}'")
"""
