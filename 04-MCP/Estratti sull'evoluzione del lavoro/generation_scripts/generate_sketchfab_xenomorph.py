...
def main():
    ...
    try:
        print("Generating professional museum-grade H.R. Giger Xenomorph (Sketchfab topology/detail style)...")
...
def create_xeno_material():
    \"\"\"Creates a professional, multi-layered glistening wet biomechanical material.\"\"\"
    ...
    node_bsdf.inputs['Base Color'].default_value = (0.010, 0.012, 0.015, 1.0)
    node_bsdf.inputs['Metallic'].default_value = 0.95
    node_bsdf.inputs['Roughness'].default_value = 0.04 # highly glossy specular
    
    # Procedural micro-scuffs and biological noise textures (Giger airbrush styling)
...
def create_dome_material():
    \"\"\"Creates a glassy, dark, high-specular translucent dome material.\"\"\"
...
def create_bone_material():
    ...
    bsdf = mat.node_tree.nodes.get("Principled BSDF")
    if bsdf:
        bsdf.inputs['Base Color'].default_value = (0.72, 0.69, 0.62, 1.0)
        bsdf.inputs['Roughness'].default_value = 0.6
        bsdf.inputs['Metallic'].default_value = 0.0
    return mat

def create_ground_material():
    \"\"\"Industrial metallic grid floor.\"\"\"
    mat = bpy.data.materials.new(name="Industrial_Floor")
    mat.use_nodes = True
    nodes = mat.node_tree.nodes
    links = mat.node_tree.links
    ...
    node_output = nodes.new('ShaderNodeOutputMaterial')
    node_bsdf = nodes.new('ShaderNodeBsdfPrincipled')
    node_bsdf.inputs['Base Color'].default_value = (0.05, 0.05, 0.06, 1.0)
    node_bsdf.inputs['Metallic'].default_value = 0.85
    node_bsdf.inputs['Roughness'].default_value = 0.18
    
    # Brick node to simulate industrial grid plate patterns
    node_brick = nodes.new('ShaderNodeTexBrick')
    node_brick.inputs['Mortar Size'].default_value = 0.015
    node_brick.inputs['Scale'].default_value = 6.0
    node_brick.inputs['Color1'].default_value = (0.05, 0.05, 0.06, 1.0)
    node_brick.inputs['Color2'].default_value = (0.07, 0.07, 0.08, 1.0)
...
def build_sketchfab_xeno(x_target, y_target, z_target, yaw_deg, xeno_mat, bone_mat, dome_mat):
    \"\"\"Builds the museum-grade H.R. Giger Xenomorph body and details.\"\"\"
    parts = []
    greebles = [] # Separate high-frequency mechanical details
    ...
    # Corrugated hoses running along top of inner skull (brain-machine interface)
    for i in range(5):
        y_hose = 0.45 - (i * 0.15)
        bpy.ops.mesh.primitive_torus_add(location=(0, y_hose, 2.12), major_radius=0.18, minor_radius=0.04)
        hose_ring = bpy.context.active_object
        hose_ring.rotation_euler = (0, math.radians(90), 0)
        hose_ring.data.materials.append(bone_mat)
    ...
    # Pharyngeal pale jaw with sharp teeth cones
    ...
    # 6 tiny sharp teeth cones at the front of inner jaw
    for i in range(3):
        x_offset = -0.04 + (i * 0.04)
        # Upper teeth
        bpy.ops.mesh.primitive_cone_add(radius1=0.015, depth=0.05, location=(x_offset, 0.9, 1.66))
        u_tooth = bpy.context.active_object
        u_tooth.rotation_euler = (math.radians(180), 0, 0)
        u_tooth.data.materials.append(bone_mat)
        # Lower teeth
        bpy.ops.mesh.primitive_cone_add(radius1=0.015, depth=0.05, location=(x_offset, 0.9, 1.58))
        l_tooth = bpy.context.active_object
        l_tooth.data.materials.append(bone_mat)
    ...
    # E. GAUNT SKELETAL LIMBS WITH CHITIN PLATES
    # Left Arm
    ...
    # Overlapping shoulder plate
    bpy.ops.mesh.primitive_cylinder_add(radius=0.08, depth=0.3, location=(-0.35, 0.05, 1.45))
    plate_sh_l = bpy.context.active_object
    plate_sh_l.scale = (1.1, 0.5, 1.0)
    plate_sh_l.rotation_euler = (math.radians(-25), math.radians(35), 0)
    parts.append(plate_sh_l)
    ...
    # Three long skeletal claw fingers
    for i in range(3):
        f_rot = math.radians(-90 + (i - 1) * 20)
        bpy.ops.mesh.primitive_cylinder_add(radius=0.012, depth=0.18, location=(-0.68, 0.72 + i * 0.04, 0.98))
        finger = bpy.context.active_object
        finger.rotation_euler = (f_rot, 0, 0)
        parts.append(finger)
    ...
    bpy.ops.mesh.primitive_cylinder_add(radius=0.08, depth=0.3, location=(0.35, 0.05, 1.45))
    plate_sh_r = bpy.context.active_object
    plate_sh_r.scale = (1.1, 0.5, 1.0)
    plate_sh_r.rotation_euler = (math.radians(-25), math.radians(-35), 0)
    parts.append(plate_sh_r)
    ...
    for i in range(3):
        f_rot = math.radians(-90 + (i - 1) * 20)
        bpy.ops.mesh.primitive_cylinder_add(radius=0.012, depth=0.18, location=(0.72, 0.72 + i * 0.04, 0.98))
        finger = bpy.context.active_object
        finger.rotation_euler = (f_rot, 0, 0)
        parts.append(finger)
        
    # Left Leg (digitigrade skeletal joints with thick overlapping chitin shells)
    ...
    # Thigh shield plate
    bpy.ops.mesh.primitive_cylinder_add(radius=0.11, depth=0.45, location=(-0.32, -0.05, 0.85))
    thigh_pl_l = bpy.context.active_object
    thigh_pl_l.scale = (1.1, 0.6, 1.0)
    thigh_pl_l.rotation_euler = (math.radians(40), math.radians(15), 0)
    parts.append(thigh_pl_l)
    ...
    bpy.ops.mesh.primitive_cylinder_add(radius=0.11, depth=0.45, location=(0.32, -0.05, 0.85))
    thigh_pl_r = bpy.context.active_object
    thigh_pl_r.scale = (1.1, 0.6, 1.0)
    thigh_pl_r.rotation_euler = (math.radians(40), math.radians(-15), 0)
    parts.append(thigh_pl_r)
    ...
    # G. BIOMECHANICAL GREEBLES: Curved hydraulic hoses (shoulder to ribs)
    # Left hydraulic conduit
    bpy.ops.mesh.primitive_torus_add(location=(-0.35, -0.08, 1.25), major_radius=0.2, minor_radius=0.02)
    hose_l = bpy.context.active_object
    hose_l.rotation_euler = (math.radians(45), math.radians(45), 0)
    parts.append(hose_l)
    
    # Right hydraulic conduit
    bpy.ops.mesh.primitive_torus_add(location=(0.35, -0.08, 1.25), major_radius=0.2, minor_radius=0.02)
    hose_r = bpy.context.active_object
    hose_r.rotation_euler = (math.radians(45), math.radians(-45), 0)
    parts.append(hose_r)
    
    # H. ORGANIC VOXEL FUSION & REMESHING OF THE BODY
    ...
    # Parent body, dome, inner skull structures, and jaws to Empty
    ...
    # Parent dental and corrugated elements to Empty
    for obj in bpy.data.objects:
        if obj.name.startswith("Leaves_Cluster") or obj.name.startswith("Leaves_Cluster.00") or obj.name.startswith("Cone") or obj.name.startswith("Cone.00") or obj.name.startswith("Torus") or obj.name.startswith("Torus.00"):
            if obj.parent is None and obj != xeno_system:
                obj.parent = xeno_system
...
def build_standalone_scene():
    ...
    # 1. Industrial Grid Ground Plane
    bpy.ops.mesh.primitive_plane_add(size=30.0, location=(0, 0, 0))
    floor = bpy.context.active_object
    floor.name = "Industrial_Grid_Floor"
    floor.data.materials.append(ground_mat)
    
    # 2. Build 1979 Big Chap Xenomorph with Sketchfab detail topology
    xeno = build_sketchfab_xeno(x_target=0.0, y_target=0.0, z_target=0.05, yaw_deg=180,
                                xeno_mat=xeno_mat, bone_mat=bone_mat, dome_mat=dome_mat)
    
    # 3. MOODY CINEMATIC STUDIO Lighting Rig (High-contrast Chiaroscuro)
    ...
    # 4. CAMERA SETUP (Sketchfab lookdev view)
    ...
    # Position to capture the close-up portrait
...
print("Professional Giger Big Chap Xenomorph with Sketchfab detail topology generated successfully!")
"""
